...
```json
    "paths": {
        "/login": {
            "post": {
                "operationId": "POST::/login::Tutorial\\Rest\\Login::mymethod",
        }
    }
```
...
With definition above every request to `POST /login` will be handled by the method `mymethod` of the class `Tutorial\Rest\Login`.
